...
class CLIMainTests(unittest.TestCase):
    ...
    def test_resume_cli_falls_back_to_pending_orchestration_state(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            project_root = Path(td) / "proj-test"
            project_root.mkdir(parents=True, exist_ok=True)
            with patch(
                ...
                with patch(
                    ...
                    return_value={
                        ...
                        "pending_orchestration": {
                            ...
                            "turn_dir": str(project_root / ".agents-inc" / "turns" / "turn-pending"),
                            "resume_from_cycle": 3,
                            "group_objectives": {"developer": "resume dev"},
                            "cycle_summaries": [{"cycle_id": 1}],
                        },
                    },
                ):
            ...
            run_chat.assert_called_once()
            cfg = run_chat.call_args.args[0]
            self.assertEqual(cfg.auto_restart_checkpoint_id, "pending-interrupted-turn")
            self.assertEqual(cfg.auto_restart_objective, "pending objective")
            self.assertEqual(int(cfg.auto_restart_from_cycle), 3)
